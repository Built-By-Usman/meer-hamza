    ...
    <div className="min-h-screen flex flex-col bg-background font-sans">
      ...
      <main className="flex-1 bg-background pb-16">
        ...
        {/* ═══════════════════════════════════════════════
            1. CINEMATIC HERO
        ═══════════════════════════════════════════════ */}
        <section className="relative w-full h-[55vw] min-h-[300px] max-h-[620px] bg-zinc-950 overflow-hidden">
          {/* Background image */}
          <OptimizedImage
            ...
            priority
            loading="eager"
            className="object-cover opacity-70 scale-[1.03] transition-transform duration-[8s] ease-out"
          />
          ...
          <div className="absolute inset-0 flex flex-col justify-center px-6 sm:px-12 lg:px-20 text-white">
            <span
              className="block font-sans text-[8px] sm:text-[10px] uppercase tracking-[0.35em] text-primary mb-3 transition-all duration-700"
              style={{
            ...
            <h1
              className="font-serif text-3xl sm:text-5xl lg:text-6xl font-light tracking-wider text-white leading-[1.1] max-w-lg transition-all duration-700"
              style={{
                opacity: heroReady ? 1 : 0,
            ...
            <p
              className="mt-4 font-sans text-[10px] sm:text-xs text-zinc-300 font-light tracking-wide max-w-xs sm:max-w-sm leading-relaxed hidden sm:block transition-all duration-700"
              style={{
                opacity: heroReady ? 1 : 0,
            ...
            <div
              className="mt-6 sm:mt-8 flex items-center gap-4 transition-all duration-700"
              style={{
                opacity: heroReady ? 1 : 0,
                transform: heroReady ? 'translateY(0)' : 'translateY(14px)',
            ...
            >
              <Link href="/categories">
                <button className="btn-shimmer font-sans text-[9px] sm:text-[10px] uppercase tracking-[0.22em] font-bold bg-primary text-primary-foreground px-6 sm:px-8 h-9 sm:h-10 flex items-center gap-2 hover:bg-primary/90 transition-colors cursor-pointer">
                ...
                <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scroll-smooth no-scrollbar lg:hidden">
                  {categories.map((cat, idx) => (
                    <Link
                      ...
                      className="snap-start flex-shrink-0 w-[150px] sm:w-[200px] relative aspect-[3/4] bg-secondary overflow-hidden group"
                    >
                      <OptimizedImage
                        src={cat.image}
                        alt={cat.name}
                ...
                <div className="hidden lg:grid grid-cols-4 grid-rows-2 gap-3 h-[500px]">
                  ...
                  {categories[0] && (
                    <Link
                      ...
                      className="col-span-2 row-span-2 relative bg-secondary overflow-hidden group"
                    >
                      <OptimizedImage
                  ...
                  {categories.slice(1, 5).map((cat) => (
                    <Link
                      key={cat.id}
                      href={`/category/${cat.slug}`}
                      className="col-span-1 row-span-1 relative bg-secondary overflow-hidden group"
                    >
                      <OptimizedImage
                        src={cat.image}
                        alt={cat.name}
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {TRUST_PILLARS.map(({ icon: Icon, title, body }) => (
              <div key={title} className="flex flex-col items-start gap-3">
                <div className="w-9 h-9 flex items-center justify-center border border-primary/20 bg-primary/5">
        ...
        <section className="mx-4 sm:mx-6 lg:mx-8 mb-4">
          <div className="relative bg-zinc-950 overflow-hidden px-8 sm:px-16 py-16 sm:py-20 text-center">
            ...
            >
              <input
                ...
                className="flex-1 bg-zinc-900 border border-zinc-700 border-r-0 px-4 py-2.5 font-sans text-xs text-white placeholder:text-zinc-500 focus:outline-none focus:border-primary/60 transition-colors"
                required
              />
              <button
                type="submit"
                className="btn-shimmer bg-primary text-primary-foreground font-sans font-bold text-[9px] uppercase tracking-[0.22em] px-7 h-10 hover:bg-primary/90 transition-colors cursor-pointer border border-primary flex-shrink-0"
              >
                Subscribe
              </button>
            </form>
