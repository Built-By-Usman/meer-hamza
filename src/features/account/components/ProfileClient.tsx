...
export function ProfileClient() {
  ...
  // Only fetch orders when the user is authenticated
        ...
        <Tabs defaultValue={activeTab} value={activeTab} onValueChange={setActiveTab} className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-3 space-y-6">
            ...
            <TabsContent value="dashboard">
                ...
                <Card className="rounded-xl border shadow-xs text-left">
                  <CardContent className="p-5 flex items-center space-x-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-bold">{isAuthenticated ? orders?.length || 0 : 0}</p>
                    </div>
                  </CardContent>
                </Card>
                <Card className="rounded-xl border shadow-xs text-left">
              ...
              {!isAuthenticated ? (
                <div className="border border-dashed rounded-xl p-8 text-center bg-card/25 max-w-lg mx-auto">
                  ...
                  <h3 className="font-semibold text-base">Sign in to view your account</h3>
                  <p className="text-xs text-muted-foreground mt-1 max-w-xs mx-auto">Sign in to track your orders, manage your wishlist and view your order history.</p>
            ...
            <TabsContent value="orders">
              {!isAuthenticated ? (
                <div className="border border-dashed rounded-xl p-8 text-center bg-card/25">
                  <UserIcon className="h-10 w-10 text-muted-foreground mx-auto mb-3" />
                  <h3 className="font-semibold text-base">Sign in to view your orders</h3>
                  <p className="text-xs text-muted-foreground mt-1 max-w-xs mx-auto">Your order history will appear here once you sign in.</p>
                  <Button onClick={() => setIsAuthModalOpen(true)} className="mt-4 rounded-md cursor-pointer font-bold">
                    Sign In
                  </Button>
                </div>
              ) : isLoadingOrders ? (
                      ...
                      <Card key={ord.id} className="rounded-xl border shadow-sm text-left overflow-hidden">
                        ...
                        <div className="bg-secondary/15 border-b p-4 sm:p-5 flex flex-wrap justify-between items-center gap-3">
                          <div className="space-y-1">
                            ...
                            <div className="text-sm font-bold text-foreground">#{ord.id.slice(0, 8).toUpperCase()}</div>
                          </div>
                          <div className="space-y-1">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="text-sm font-extrabold text-foreground">{formatPrice(ord.total)}</div>
                          </div>
                        </div>
                        ...
                        <div className="p-5 space-y-4">
                          ...
                          <div className="border-t pt-4 flex flex-wrap items-center justify-between text-xs text-muted-foreground gap-3">
                            ...
                            <div className="flex items-center space-x-1 font-semibold">
                              ...
                              <span>Est Delivery: 4 to 5 business days</span>
                            </div>
                          </div>
                        </div>
                      </Card>
