...
export function ProductReviews({
  ...
  const ratingBreakdown = [5, 4, 3, 2, 1].map((r) => {
    ...
    const percent = reviewCount > 0 ? (matching.length / reviewCount) * 100 : 0;
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1.5 font-sans text-xs">
            {reviewCount > 0 ? (
              <>
                <Rating rating={averageRating} size={11} />
                <span className="text-muted-foreground">({averageRating} average out of {reviewCount} review{reviewCount === 1 ? '' : 's'})</span>
              </>
            ) : (
              <span className="text-muted-foreground">No reviews yet — be the first to share your experience.</span>
            )}
          </div>
        </div>
