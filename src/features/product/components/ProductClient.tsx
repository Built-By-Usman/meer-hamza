  ...
  const handleReviewSubmit = (rating: number, comment: string, authorInput: string) => {
    ...
    addReviewMutation.mutate(
      ...
      },
      {
        ...
        onError: (err: any) => {
          toast.error(err?.info?.detail || err?.message || 'Error submitting review');
        },
      }
    );
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 lg:gap-14 text-left">
          ...
          <div ref={mainPurchasePanelRef} className="md:col-span-5 flex flex-col justify-start space-y-6">
            ...
            <div className="py-2 border-y border-border/10 flex items-baseline gap-3">
              <span className="text-2xl font-bold text-foreground font-sans">{formatPrice(price)}</span>
              {originalPrice > price && (
                <span className="text-sm text-muted-foreground line-through font-sans">{formatPrice(originalPrice)}</span>
              )}
              {discountPercent > 0 && (
            ...
            {(() => {
              ...
              // Use the actual attribute key name (e.g. "Size", "Volume", "Fragrance") as heading
              ...
              return (
                <div className="space-y-2">
                  <span className="text-[8px] font-bold text-muted-foreground uppercase tracking-widest block font-sans">Select {heading}</span>
            ...
            <div className="space-y-3 pt-2">
              ...
              <Button
                ...
                className="w-full h-11 font-bold rounded-xl uppercase text-[10px] tracking-widest cursor-pointer flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90 shadow-md"
              >
            ...
            <Accordion type="single" className="w-full border-t border-border/10 pt-2">
              ...
              <AccordionItem value="shipping" className="border-b border-border/10 py-1">
                ...
                <AccordionContent className="font-sans text-xs leading-relaxed text-muted-foreground pt-1 pb-3 text-left space-y-1.5">
                  ...
                  <p>Free standard courier shipping globally for orders above Rs. 15,000.</p>
                </AccordionContent>
              </AccordionItem>
            </Accordion>
            ...
            <div className="max-w-xl mx-auto flex items-center justify-between gap-4 font-sans bg-zinc-950 text-white rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)] border border-white/8 px-4 py-2.5">
              <div className="flex items-center space-x-3 overflow-hidden">
                ...
                <div className="text-left overflow-hidden">
                  ...
                  <p className="text-[9px] text-zinc-400 truncate">{selectedVariant ? (Object.values(selectedVariant.attributes || {}).find(v => !!v) || '') : ''}</p>
                </div>
              </div>
              
              <div className="flex items-center space-x-3.5">
                <span className="text-xs font-bold text-white">{formatPrice(price)}</span>
                <button 
                  onClick={() => {
                    if (quantityInCart > 0) {
