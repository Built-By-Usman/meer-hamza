...
export function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  ...
  const { items, subtotal, discount, shippingCost, total, updateQuantity, removeFromCart } = useCartStore();
  ...
  return (
    <Sheet isOpen={isOpen} onClose={onClose} side="right">
      ...
      {items.length > 0 && (
        <SheetFooter className="p-6 border-t flex flex-col space-y-4 bg-secondary/15">
          {/* Summary pricing */}
          <div className="space-y-1.5 text-sm w-full">
            ...
            <div className="flex justify-between text-base font-bold text-foreground border-t pt-2 mt-2">
              <span>Total</span>
          ...
          <div className="grid grid-cols-2 gap-2 w-full">
            <Button variant="outline" onClick={handleViewCartClick} className="w-full cursor-pointer">
              View Cart
            </Button>
            <Button onClick={handleCheckoutClick} className="w-full cursor-pointer flex items-center justify-center space-x-2">
